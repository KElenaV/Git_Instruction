...
Очень осторожно использовать эту функцию т.к. файлы после нее также возвращаются к тому состоянию, в котором были в момент возвращенного коммита или даже удалится, если его не было на тот момент. Есть риск удалить что-то нужное.
